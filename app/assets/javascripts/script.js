	...
	$(document).ready(function(){
		...
		$('.attr-color').hover(function(){
			...
			$(this).next().find('select').find('option').each(function(){
				...
				var el_class = '';
				if(current_val == val) el_class = ' class="selected" ';
				if(val != '') content += '<li data-attribute="'+val+'"><a href="#"'+el_class+'><span class="color-'+val+' fa-2x"></span></a></li>';
			})
		...
		if($('.sv-slider').length>0){
			$('.sv-slider').each(function(){
				...
				if(nav == 'home-direct-nav' || nav == 'info-expert' || nav == 'content-owl-speaker'){
					pagination = false;
					navigation = true;
					prev_text = '<span class="fa fa-angle-left fa-2x"></span>';
					next_text = '<span class="fa fa-angle-right fa-2x"></span>';
				}
				if(nav == 'event-banner-slider'){
					pagination = false;
					navigation = true;
					prev_text = '<span class="fa fa-fa-angle-left fa-2x"></span>';
					next_text = '<span class="fa fa-angle-right fa-2x"></span>';
				}
		...
		if($('.event-upcomming-slider').length>0){
			$('.event-upcomming-slider').each(function(){
				$(this).find('.wrap-item').owlCarousel({
					...
					pagination: false,
					navigation: true,
					navigationText:['<span class="fa fa-angle-left fa-2x"></span>','<span class=fa fa-angle-right fa-2x"></span>']
				});
			});
		}
		//End
		...
		if($('.post-format-slider').length>0){
			$('.post-format-slider').each(function(){
				$(this).find('.wrap-item').owlCarousel({
					...
					pagination: false,
					navigation: true,
					navigationText:['<span class="fa fa-fa-angle-left fa-2x"></span>','<span class="fa fa-angle-right fa-2x"></span>']
				});
			});
		}
		//End
		...
		if($('.content-bx-latest-post').length>0){
			if($(window).width()>480){
				$('.content-bx-latest-post').bxSlider({
					...
					pager: false,
					touchEnabled:true,
					nextText:'<span class="fa fa-angle-right fa-2x"></span>',
					prevText:'<span class="fa fa-angle-left fa-2x"></span>',
				});
			}else{
				$('.content-bx-latest-post').bxSlider({
					minSlides: 1,
